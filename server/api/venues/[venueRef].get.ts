...
const VOLUME_FROM_DESCRIPTION_KEYWORDS = ["standard", "can", "bottle", "glass", "pitcher"];
const KNOWN_VOLUMES = [25, 50, 125, 175, 250, 284, 330, 440, 500, 568, 750, 1000];
...
function resolveVolumeMl(
	...
	if (item.name in SPRITZ_VOLUME_OVERRIDES) return SPRITZ_VOLUME_OVERRIDES[item.name];
	...
	if (units !== null) {
		...
		return KNOWN_VOLUMES.reduce((a, b) => (Math.abs(a - estimate) < Math.abs(b - estimate) ? a : b));
	}
...
function buildAlePriceMap(menus: Menu[]): Record<number, Record<string, number>> {
				...
				for (const item of itemGroup.items) {
					if (item.itemType === "product") {
						...
						for (const option of item.options.portion.options) {
							...
							addDrink({
								costPerUnit: costPerUnit(option.value.price.value, abv, volumeMl),
								itemName: item.name,
							...
							for (const linked of item.options.linked ?? []) {
								...
								const multiplePrice = parseFloat(tokens[tokens.length - 1].slice(1));
								addDrink({
									costPerUnit: costPerUnit(multiplePrice, abv, volumeMl * multiplier),
									itemName: item.name,
						...
						for (const [optionName, price] of Object.entries(options)) {
							const volumeMl = resolveVolumeMl(
								...
								{ description: "", name: item.fullName } as unknown as ProductItem,
								category.name,
								itemGroup.name ?? "",
								optionName,
								abv,
...
export default defineEventHandler(async (event) => {
	...
	if (isNaN(venueRef)) throw createError({ statusCode: 400, message: "Invalid venueRef" });
	...
	if (!summary) throw createError({ statusCode: 404, message: "Venue not found" });

	const venue = await getVenue(summary);
	const menuSummaries = await allMenus(venue);
	const menus = await Promise.all(menuSummaries.map((ms) => getMenu(venue, ms)));
